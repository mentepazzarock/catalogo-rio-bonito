...
import { Heart, Star, Sparkles, MapPin, ArrowRight, Building2, ChevronRight } from 'lucide-react'

export function CTASection() {
  return (
    <>
      {/* ═══ MOBILE: Single thin banner ═══ */}
      <section className="sm:hidden px-4 pb-4">
        <Link
          href="/cadastro-empresa"
          className="flex items-center gap-3 rounded-lg bg-primary-50 border border-primary-100 p-3 active:bg-primary-100 transition-colors"
        >
          <Building2 className="h-5 w-5 text-primary-600 shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold text-slate-900">Tem um negócio? Cadastre grátis</p>
          </div>
          <ChevronRight className="h-4 w-4 text-slate-400 shrink-0" />
        </Link>
      </section>

      {/* ═══ DESKTOP: Full gradient CTA ═══ */}
      <section className="hidden sm:block py-16 sm:py-20 bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900 relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center flex flex-col items-center">
            ...
            <div className="mt-12 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-center gap-2">
              <span className="text-sm text-primary-200">Você é dono(a) de um negócio?</span>
              <Link
                href="/cadastro-empresa"
