...
export function BusinessListPage({ businesses, categories, title, subtitle, icon, emptyMessage }: BusinessListPageProps) {
            ...
            <div className="mt-3 animate-scale-in">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-700 focus:outline-none">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex bg-slate-100 rounded-lg p-0.5">
            <button onClick={() => setViewMode('list')} className={`flex items-center gap-1 px-2.5 py-1.5 text-xs rounded-md transition-colors ${viewMode === 'list' ? 'bg-white text-slate-900 shadow-sm font-semibold' : 'text-slate-500'}`}>
              <List className="h-3.5 w-3.5" /><span className="hidden sm:inline">Lista</span>
            </button>
            <button onClick={() => setViewMode('grid')} className={`flex items-center gap-1 px-2.5 py-1.5 text-xs rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white text-slate-900 shadow-sm font-semibold' : 'text-slate-500'}`}>
        ...
        {filteredBusinesses.length > 0 ? (
          ...
          <EmptyState icon={Search} title="Nenhum resultado" description={emptyMessage}
            action={<button onClick={clearFilters} className="text-sm font-semibold text-primary-600 hover:text-primary-700">Limpar filtros e ver todos</button>}
          />
        )}
      </div>
    </div>
