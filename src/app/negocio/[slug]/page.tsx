...
export async function generateStaticParams() {
  return mockBusinesses.map((b) => ({ slug: b.slug }))
}
...
export default async function BusinessPage({ params }: PageProps) {
  ...
  const activePromotions = business.promotions.filter((p) => p.is_active)
  const featuredProducts = business.products.filter((p) => p.is_featured)
  const featuredServices = business.services.filter((s) => s.is_featured)
          ...
          <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-6 sm:p-8 mb-6">
            <div className="flex flex-col sm:flex-row gap-5">
              ...
              <div className={`flex h-20 w-20 sm:h-24 sm:w-24 shrink-0 items-center justify-center rounded-2xl border-4 border-white shadow-lg bg-gradient-to-br ${gradientClass}`}>
                <span className="text-2xl sm:text-3xl font-extrabold text-white mix-blend-overlay">
                  {business.name.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase()}
                </span>
                <span className="text-2xl sm:text-3xl font-extrabold text-white absolute">
                  {business.name.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase()}
                </span>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-start gap-2 mb-2">
                  ...
                  {business.is_verified && (
                    <BadgeCheck className="h-6 w-6 text-primary-500 shrink-0 mt-1" />
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  ...
                  {business.is_featured && (
                    <Badge variant="warning" size="md">Destaque</Badge>
                  )}
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-slate-100">
              {business.whatsapp && (
                <a
                  href={formatWhatsAppUrl(business.whatsapp, `Olá! Vi o ${business.name} no ${SITE_NAME} e gostaria de mais informações.`)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 rounded-xl bg-accent-500 px-4 py-3 text-sm font-bold text-white hover:bg-accent-600 transition-colors shadow-sm"
                >
              ...
              {business.phone && (
                <a
                  ...
                  className="flex items-center justify-center gap-2 rounded-xl bg-primary-600 px-4 py-3 text-sm font-bold text-white hover:bg-primary-700 transition-colors shadow-sm"
                >
              ...
              {business.latitude && business.longitude && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-200 transition-colors"
                >
                      ...
                      <div key={promo.id} className="rounded-xl bg-white p-4 border border-accent-200 shadow-sm">
                        ...
                        {promo.coupon_code && (
                          <div className="flex items-center gap-2">
                            ...
                            <code className="rounded-md bg-accent-100 px-2 py-1 text-sm font-bold text-accent-700">
                              {promo.coupon_code}
                            </code>
                          </div>
                        )}
                ...
                <section className="rounded-2xl bg-white border border-slate-200 p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {business.products.map((product) => (
                      <div
                        key={product.id}
                        className={`rounded-xl border p-4 transition-colors hover:border-slate-300 ${product.is_featured ? 'border-warm-200 bg-warm-50/50' : 'border-slate-100'
                          }`}
                      >
                        <div className="flex items-start justify-between">
                          <div>
                            <h3 className="text-sm font-bold text-slate-900">
                              {product.name}
                              {product.is_featured && (
                                <Star className="inline h-3 w-3 text-warm-500 fill-warm-500 ml-1" />
                              )}
                            </h3>
                            {product.description && (
                ...
                <section className="rounded-2xl bg-white border border-slate-200 p-6">
                  ...
                  <div className="space-y-3">
                    {business.services.map((service) => (
                      <div
                        key={service.id}
                        className={`flex items-center justify-between rounded-xl border p-4 transition-colors hover:border-slate-300 ${service.is_featured ? 'border-primary-200 bg-primary-50/50' : 'border-slate-100'
                          }`}
                      >
                        <div>
                          <h3 className="text-sm font-bold text-slate-900">
                            {service.name}
                            {service.is_featured && (
                              <Badge variant="info" size="sm" className="ml-2">Popular</Badge>
                            )}
                          </h3>
                          {service.description && (
                          ...
                          {business.whatsapp && (
                            <a
                              href={formatWhatsAppUrl(
                                business.whatsapp,
                                `Olá! Gostaria de agendar o serviço "${service.name}" que vi no ${SITE_NAME}.`
                              )}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="block mt-1 text-xs font-semibold text-accent-600 hover:text-accent-700 hover:underline"
                            >
                              Agendar
                            </a>
                          )}
              ...
              <section className="rounded-2xl bg-white border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-bold text-slate-900">
                    Avaliações ({business.total_reviews})
                  </h2>
                  <div className="flex items-center gap-2">
              ...
              <div className="rounded-2xl bg-white border border-slate-200 p-6">
                ...
                {business.latitude && business.longitude && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 rounded-xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-200 transition-colors justify-center"
                  >
                    <Navigation className="h-4 w-4" />
                    Obter Rota via Google
                  </a>
                )}
              </div>
            </MotionWrapper>
                ...
                <div className="space-y-2">
                  {DAYS_OF_WEEK.map(({ key, label }) => {
                    ...
                    return (
                      <div
                        key={key}
                        className={`flex items-center justify-between py-1.5 text-sm transition-colors ${isToday ? 'font-semibold text-primary-700 bg-primary-50 px-2 -mx-2 rounded-md' : 'text-slate-600'
                          }`}
                      >
      ...
      {business.whatsapp && (
        <div className="fixed bottom-20 right-4 z-40 md:hidden animate-scale-in">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex h-14 w-14 items-center justify-center rounded-full bg-accent-500 text-white shadow-lg shadow-accent-500/30 hover:bg-accent-600 transition-colors active:scale-95"
          >
